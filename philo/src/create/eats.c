...
/*   Updated: 2023/01/14 20:49:14 by fnieves-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	...
	In argument 1: the philosopher i struct and the time at which they started eating.
	Argument 2: A pthread_mutex_t, which is used for synchronization to ensure that 
	only one philosopher can acquire a fork at a time.
...
/*
	This function is used to acquire left or right forks needed for a philosopher to eat.
	It converts to "time_eating" the instant at which the philosopher started eating in ms.
	It then enters in a while loop that calls the fork_takeone() function to try and acquire a fork.
	This loop continues until fork_takeone() returns 1, indicating that the fork has
	been successfully acquired.
	It then checks if the time at which the philosopher started eating plus the time the philosopher
	can live before dying is less than the current time.
	If it is, it means the philosopher has died of starvation, and the function sets the philosopher's
	status to DIEDS and returns 0.
