...
/*   Updated: 2023/01/14 21:52:42 by fnieves-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	The function is used to print a message indicating the time, 
	philosopher id and status that user sees.
	...
	It stores the value of the running variable in the t_philos struct
	in a local variable 'running'. It then checks if the running variable
	is true, if so it prints the message status with the time and philosopher
	id that user sees. It then unlocks the mutexes and returns the value of 
	the running variable. If in the middel of the function dinner stops , 
	it will return 0, and will not print.
*/
...
int	gettimediff(struct timeval *t)
{
	...
	diff_time = ((now.tv_sec - t->tv_sec) * 1000 \
...
/*
	Is used to wait for a certain amount of time (the time it takes
	the philosopher to eat) to pass. Will change the status to DIED
	if time to died is consummed since start eating. It starts by calling 
	the get_actual_time() function to get the current time and stores it
	in a variable 'start'. It also gets the time at which the philosopher
	...
	Inside the while loop it calls the usleep(100) function to sleep
	for 100 microseconds and then updates
*/
